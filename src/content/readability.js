...
(() => {
  ...
  // src/content/extract/article-core.ts
  var import_readability = __toESM(require_readability(), 1);
  function readArticle(doc, url) {
  ...
  // extension/src/content/readable-entry.ts
  window.__eggReadable = {
    extract() {
      ...
      if (article && article.text.trim().length > 0) {
        return {
          kind: "article",
          text: article.text.slice(0, 5e4),
          html: article.html.slice(0, 2e5),
          title: article.title,
      ...
      return {
        kind: "fallback",
        text: document.body?.innerText?.slice(0, 5e4) || "",
        html: null
      };
    }
  };
